...
//34ef0008->34ef00(0000)(1000)->address becomes 14 bits long
string funct(unordered_map<char, string> hexa_to_binary, string hexa_address)
{
    string comparable_address = hexa_address.substr(0, 6) + hexa_to_binary[hexa_address.at(6)] + hexa_to_binary[hexa_address.at(7)];
...
{
    unordered_map<string, string> DM_cache_64_32;//64 KB cache size and 32 bytes cache line size
    ...
    for(ull i = 0; i < addresses_from_file.size(); i++)
    {
        ...
        //For a 64 KB cache and 4 bytes Cache line size, 
        //tag = 16 binary bits(4 hexadecimal bits)
        //index = 11 binary bits(2 hexadecimal bits + 3 binary bits)
        //byte_offset = 5 binary bits(5 binary bits)
        
        tag = addr.substr(0, 4);//4 characters after 0th position
        index = addr.substr(4, 5);//5 characters after 4th position
        byte_offset = addr.substr(9, 5);//5 characters after 9th position
        
        if(DM_cache_64_32.find(index) == DM_cache_64_32.end())// if the valid bit of index is 0, then in our map, 
                                                            // the index won't be found. Hence it is a Cache miss.
        {
            misses++;//increment the number of misses
            DM_cache_64_32.insert({index, tag});//store the index with the tag in the cache(map)
        }
        
        else// if the valid bit of the index is 1, then the index is found in our map
        {
            unordered_map<string, string> :: iterator IT = DM_cache_64_32.find(index);//initialise an iterator of the type unordered_map
            ...
            {
                misses++;
                DM_cache_64_32[index] = tag;//update the tag corresponding to that index
            }
        }
    }
