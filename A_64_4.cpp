...
{
    unordered_map<string, string> DM_cache_64_4;//64 KB cache size and 4 bytes cache line size
    ...
    for(ull i = 0; i < addresses_from_file.size(); i++)
    {
        ...
        //index = 14 binary bits(3 hexadecimal bits + 2 binary bits)
        ...
        byte_offset = addr.substr(9, 2);//2 characters after 9th position
        
        if(DM_cache_64_4.find(index) == DM_cache_64_4.end())// if the valid bit of index is 0, then in our map, 
                                                            // the index won't be found. Hence it is a Cache miss.
        {
            misses++;//increment the number of misses
            DM_cache_64_4.insert({index, tag});//store the index with the tag in the cache(map)
        }
        
        else// if the valid bit of the index is 1, then the index is found in our map
        {
            unordered_map<string, string> :: iterator IT = DM_cache_64_4.find(index);//initialise an iterator of the type unordered_map
            ...
            {
                misses++;
                DM_cache_64_4[index] = tag;//update the tag corresponding to that index
            }
        }
    }
