...
//34ef0008->34ef0008->adress remains 8 bits long
string funct(unordered_map<char, string> hexa_to_binary, string hexa_address)
{
    string comparable_address = hexa_address.substr(0, 8);
...
{
    unordered_map<string, string> DM_cache_64_1;//64 KB cache size and 1 bytes cache line size
    ...
    for(ull i = 0; i < addresses_from_file.size(); i++)
    {
        ...
        //For a 64 KB cache and 1 byte Cache line size, 
        //tag = 16 binary bits(4 hexadecimal bits)
        //index = 16 binary bits(4 hexadecimal bit), no need of byte offset
        
        tag = addr.substr(0, 4);//4 characters after 0th position
        index = addr.substr(4, 4);//4 characters after 4th position
        
        if(DM_cache_64_1.find(index) == DM_cache_64_1.end())// if the valid bit of index is 0, then in our map, 
                                                            // the index won't be found. Hence it is a Cache miss
        {
            misses++;//increment the number of misses
            DM_cache_64_1.insert({index, tag});//store the index with the tag in the cache(map)
        }
        
        else// if the valid bit of the index is 1, then the index is found in our map
        {
            unordered_map<string, string> :: iterator IT = DM_cache_64_1.find(index);//initialise an iterator of the typr unordered_map
            ...
            {
                misses++;
                DM_cache_64_1[index] = tag;//update the tag corresponding to that index
            }
        }
    }
