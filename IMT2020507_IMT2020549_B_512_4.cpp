...
//34ef0008->34e(1111)(0000)00(1000)->address becomes 17 bits long
string funct(unordered_map<char, string> hexa_to_binary, string hexa_address)
{
    string comparable_address = hexa_address.substr(0, 3) + hexa_to_binary[hexa_address.at(3)] + hexa_to_binary[hexa_address.at(4)] + hexa_address.substr(5, 2) + hexa_to_binary[hexa_address.at(7)];
...
{
    unordered_map<string, string> DM_cache_512_4;//512 KB cache size and 4 bytes cache line size
    ...
    for(ull i = 0; i < addresses_from_file.size(); i++)
    {
        ...
        //For a 512 KB cache and 4 bytes Cache line size, 
        //tag = 13 binary bits(3 hexadecimal bits + 1 binary bit)
        //index = 17 binary bits(3 binary bits + 4 binary bits + 2 hexadecimal bits + 2 binary bits)
        //byte_offset = 2 binary bits(2 binary bits)
        
        tag = addr.substr(0, 4);//4 characters after 0th position
        index = addr.substr(4, 11);//11 characters after 4th position
        byte_offset = addr.substr(15, 2);//5 characters after 9th position
        
        if(DM_cache_512_4.find(index) == DM_cache_512_4.end())// if the valid bit of index is 0, then in our map, 
                                                            // the index won't be found. Hence it is a Cache miss.
        {
            misses++;//increment the number of misses
            DM_cache_512_4.insert({index, tag});//store the index with the tag in the cache(map)
        }
        
        else// if the valid bit of the index is 1, then the index is found in our map
        {
            unordered_map<string, string> :: iterator IT = DM_cache_512_4.find(index);//initialise an iterator of the type unordered_map
            ...
            {
                misses++;
                DM_cache_512_4[index] = tag;//update the tag corresponding to that index
            }
        }
    }
