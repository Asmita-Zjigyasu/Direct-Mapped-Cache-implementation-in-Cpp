...
{
    unordered_map<string, string> DM_cache_64_16;//64 KB cache size and 16 bytes cache line size
    ...
    for(ull i = 0; i < addresses_from_file.size(); i++)
    {
        ...
        //index = 12 binary bits(3 hexadecimal bits)
        //byte_offset = 4 binary bits(4 binary bits)
        
        tag = addr.substr(0, 4);//4 characters after 0th position
        index = addr.substr(4, 3);//3 characters after 4th position
        byte_offset = addr.substr(7, 4);//4 characters after 7th position
        
        if(DM_cache_64_16.find(index) == DM_cache_64_16.end())// if the valid bit of index is 0, then in our map, 
                                                            // the index won't be found. Hence it is a Cache miss.
        {
            misses++;//increment the number of misses
            DM_cache_64_16.insert({index, tag});//store the index with the tag in the cache(map)
        }
        
        else// if the valid bit of the index is 1, then the index is found in our map
        {
            unordered_map<string, string> :: iterator IT = DM_cache_64_16.find(index);//initialise an iterator of the type unordered_map
            ...
            {
                misses++;
                DM_cache_64_16[index] = tag;//update the tag corresponding to that index
            }
        }
    }
